...
Для применения `rgba16float` требуется поддержка `texture-formats-tier2`. Сейчас она не обеспечена и не проверяется; разумно предусмотреть fallback.
...
#### Уровень Blur определяется состоянием столбца
